      ...
      >
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          {(leftLinks || middleLinks || rightLinks) && (
            <div className={`border ${colors.border.primary} ${colors.bg.tertiary} p-4 mb-8 rounded w-full max-w-full`}>
              <div className="flex flex-col sm:flex-row gap-4 w-full">
                {/* Left column */}
                <div className={`space-y-1 flex-1 sm:border-r ${colors.border.primary} pr-4`}>
                  {leftLinks?.map((link, i) => (
                    <a
                      key={i}
                      href={link.href}
                      className={`block ${colors.text.tertiary} ${colors.text.accentHover} ${colors.transition.colors} text-sm md:text-base`}
                    >
                      <span className={colors.text.linkArrow}>→</span> {link.label}
                    </a>
                  ))}
                </div>

                {/* Middle column */}
                <div className={`space-y-1 flex-1 sm:border-r ${colors.border.primary} px-4`}>
          ...
          {/* Footer */}
          <div className={`text-center border-t ${colors.border.primary} pt-6`}>
